...
export class DiggingEstimator {
  private vinRepo: IVinRepository;
  constructor(vinRepo?: IVinRepository) {
    this.vinRepo = vinRepo || new VinRepository();
  }
  ...
  protected get(rockType: string) : number[] {
    return this.vinRepo.get(rockType);
  }
}
...
class VinRepository implements IVinRepository {
  get(rockType: string): number[] {
